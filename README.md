...
MOCKA's backend was generated by a proprietary backend generator. This backend only supports the 32-bit Intel x86 architecture and the front-end was designed to map to a 32-bit IR. In order to support any modern 64-bit architecture, a large part of the front-end would need to be rewritten, and a new backend would need to be designed and generated. This would take considerable time and effort, only to result in a compiler for legacy code that is written in an outdated dialect of Modula-2. The outcome would not justify the effort.
